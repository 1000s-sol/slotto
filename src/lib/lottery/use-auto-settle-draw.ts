...
/**
 * Retries after sales close. force_settle usually finishes in one pass;
 * Switchboard reveal (if ever restored) may need a second.
 */
const SETTLE_INTERVAL_MS = 6_000;
const VRF_SETTLE_INTERVAL_MS = 8_000;
const CRANK_BACKOFF_MS = 15_000;

/**
 * When the countdown hits zero, drive settlement via the server keeper until
 * the draw reaches Settled or Refunded (no wallet popups). Server throttle
 * collapses concurrent homepage / admin / cron cranks for the same draw.
 * Admin Settle remains a manual backup.
 */
    ...
    const tick = async () => {
      ...
      try {
        ...
        if (
          !cancelled &&
          drawRef.current &&
          !drawTerminalState(drawRef.current)
        ) {
          schedule(intervalMs);
        }
      }
